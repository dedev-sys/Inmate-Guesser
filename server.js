...
app.post("/random-case", async (req, res) => {
  ...
  try {
    ...
    function getDangerScore(offense) {
      ...
      if (exactScores[normalized] !== undefined) {
        return exactScores[normalized];
      }

      // Fallback keyword-based danger scoring
      if (normalized.includes("MURDER")) return 95;
      if (normalized.includes("KIDNAP")) return 88;
      if (normalized.includes("ARMED ROBBERY")) return 78;
      if (normalized.includes("ROBBERY")) return 62;
      if (normalized.includes("ASSAULT")) return 72;
      if (normalized.includes("ARSON")) return 55;
      if (normalized.includes("BURGLARY")) return 28;
      if (normalized.includes("FRAUD")) return 12;
      if (normalized.includes("THEFT")) return 18;
      if (normalized.includes("DRUG")) return 35;
      if (normalized.includes("WEAPON")) return 58;

      // Unknown crimes get a middle fallback
      return 45;
    }
